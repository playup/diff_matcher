  ...
  class Difference
    ...
    def difference_to_s(expected, actual)
      ...
      if match
        ...
        "#{markup(:missing, expected.inspect, expected.class)}#{markup(:additional, actual.inspect, expected)}"
        if   actual    == :___null
          "#{markup(:missing, expected.inspect, expected)}"
        elsif expected == :___null
          "#{markup(:additional, actual.inspect, expected)}" unless @quiet
        elsif match_type == :match_matcher
          d
        else
          "#{markup(:missing, expected.inspect, expected)}#{markup(:additional, actual.inspect, expected)}"
        end
      end
    end

    def markup(item_type, item, expected)
